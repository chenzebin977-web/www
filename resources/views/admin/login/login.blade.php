    ...
    <div class="col-md-3 col-sm-3 col-12 ">
        <div class="login-page">
            ...
            <div class="auth-brand text-center text-lg-left">
               {{--<img src="/uploads/{!! get_setting_value('titles_logo','website') !!}" width="35"> --}}&nbsp;运营后台
            </div>
                        ...
                        <form id="login-form" method="POST" action="{{ admin_url('auth/login') }}">
                            ...
                            <fieldset class="form-label-group form-group position-relative has-icon-left">
                                <input
                                    id="google_code"
                                    type="text"
                                    class="form-control {{ $errors->has('google_code') ? 'is-invalid' : '' }}"
                                    name="google_code"
                                    placeholder="{{ trans('admin.google_code') }}"
                                    autocomplete="current-password"
                                >
                                ...
                                <label for="google_code">{{ trans('admin.google_code') }}</label>

                                <div class="help-block with-errors"></div>
                                @if($errors->has('google_code'))
                                    <span class="invalid-feedback text-danger" role="alert">
                                                    @foreach($errors->get('google_code') as $message)
                                            <span class="control-label" for="inputError"><i class="feather icon-x-circle"></i> {{$message}}</span><br>
                                        @endforeach
                                                    </span>
                                @endif
